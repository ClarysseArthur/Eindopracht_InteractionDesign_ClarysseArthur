...
namespace SatelliteAPI.Repositories
{
    public static class SatelliteRepository
    {
        ...
        public static async Task<Satellite> GetSatelliteBackUp()
        {
            using (HttpClient client = GetClient())
            {
                try
                {
                    string url = "https://api.n2yo.com/rest/v1/satellite/tle/25544&apiKey=589P8Q-SDRYX8-L842ZD-5Z9";
        ...
        public static async Task<SatellitesAbove.Rootobject> GetAbove(string lat, string lon)
        {
            using (HttpClient client = GetClient())
            {
                try
                {
                    string url = $"https://api.n2yo.com/rest/v1/satellite/above/{lon}/{lat}/10/20/0&apiKey=WNQB9D-3FHVUW-L6SJKF-4SFS";
